...
//    private Handler handler;
    ...
    private final Runnable syncRunnable = new Runnable() {
        @Override
        public void run() {
            syncNextLocation(); // Your method to sync location
            handler.postDelayed(this, 120000); // Repeat every 120 seconds
        }
    };
...
//            syncUnsyncedLocations(getApplicationContext());
//        }
...
//        handler = new Handler(Looper.getMainLooper()); // Get main thread handler
        ...
        handler.post(syncRunnable); // Start the repeating task
...
//                                Toast.LENGTH_SHORT).show());
                        ...
                        if (distanceEstimate > 0.1) {
                            ...
                            dbHelper.insertLocation(location.getLatitude()+"",
                                    location.getLongitude()+"",
                                    location.getAccuracy()+"",
                                    distanceEstimate+"","unsynced",
                                    (mTripId==null?null:Integer.parseInt(mTripId))
                                    ,"movement",null);
...
//
//                                if (isInternetAvailable()) {
//                                    Log.d("LocationService", "Internet is available");
//
...
//                                } else {
//                                    dbHelper.insertLocation(location.getLatitude()+"",
//                                            location.getLongitude()+"",location.getAccuracy()+"",distanceEstimate+"","unsynced");
//
//                                    Log.d("LocationService", "No internet connection");
...
//        Mylocationdatabasehelper db = new Mylocationdatabasehelper(context);
        List<Locationmodel> unsynced = dbHelper.getUnsyncedLocations();
                            ...
                            try {
                                ...
                                if (result != null && result.contains("Location") ){
                                    ...
                                    if (jsonObject.has("status") &&
                                        ...
                                        dbHelper.markLocationAsSynced(location.id);
...
//            });
    ...
    private void syncNextLocation() {
        Locationmodel location = dbHelper.getFirstUnsyncedLocation(); // query single item from DB
        ...
        try {
            requestBody.put("lat", location.lat);
            requestBody.put("long", location.lon);
            requestBody.put("trip_id", location.tripId==0?null:location.tripId);
            requestBody.put("accuracy", location.accuracy);
            requestBody.put("distance", location.distance);
            requestBody.put("type", location.type);
...
//                            syncNextLocation();
                    ...
                    Log.d("Exception message: ",response.message());
                }
            }
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                ...
                Log.d("Exception message: ",t.getMessage());
            }
        });
    }
